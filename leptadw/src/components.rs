...
pub fn Desktop(children: Children) -> impl IntoView {
        view! {
            <div class="conainer mx-auto  w-screen h-screen flex items-center justify-center desktop">
                {children()}
            </div>
        }
}
...
pub fn Img(
        ...
        view! { <img class="rounded-lg" src=src alt=alt/> }
}

#[component]
pub fn Label(children: Children) -> impl IntoView {
        view! { <label class="text-neutral-200">{children()}</label> }
}
...
pub fn SelectMenu(
        ...
        view! {
            <div class="relative">
                <button
                    class="relative w-full cursor-default rounded-lg bg-neutral-800 text-neutral-200 py-1.5 pl-3 \
                    pr-10 text-left shadow-inner shadow-neutral-900 focus:outline focus:outline-2 \
                ...
                <ul
                    class="absolute z-10 mt-1 max-h-56 w-full overflow-auto rounded-lg bg-neutral-800 py-1 text-base \
                ...
                >
                    ...
                    >
                        ...
                        <div class="flex items-center">
                            <span class="text-neutral-500 font-normal ml-3 block truncate">
                                Select a/an {label} ...
                            </span>
                        </div>
...
pub fn CityActionRow<T: FnMut(leptos::ev::MouseEvent) + 'static>(
        ...
        #[prop(default = String::from("angle-right"))] icon: String,
) -> impl IntoView {
...
pub fn EntryRow<T: FnMut(leptos::ev::Event) + 'static>(
        #[prop(default = String::from("Input Text"))] label: String,
        on_input: T,
        value: ReadSignal<String>,
) -> impl IntoView {
        ...
        view! {
            <div class="relative">
                ...
                <label
                    for=id
                    class="absolute text-md text-neutral-400 duration-300 transform -translate-y-4 \
...
#[component]
pub fn NavigationView(#[prop(optional)] class: String, children: Children) -> impl IntoView {
        let class = format!("NavigationView {}", class);
...
pub fn NavigationPage(
        ...
        #[prop(default = false)] prev: bool,
        #[prop(default = 0)] page: i8,
        #[prop(optional)] current_page: Option<Box<dyn Fn() -> i8>>,
        #[prop(optional)] on_back: Option<Box<dyn Fn()>>,
        children: Children,
) -> impl IntoView {
        let class = format!("NavigationPage {}", class);
        let (visible, set_visible) = create_signal(false);

        create_effect(move |_| {
                current_page.as_ref().map(|f| {
                        if page <= f() {
                                set_visible(true);
                                return;
                        }
        ...
        view! {
            <div class=class class:CurrentPage=move || { visible() }>
                        ...
                        .map_or_else(
                            || {
                                view! {
                                    <div class="flex items-center">
                                        <div
                                            class="ml-3 p-1 rounded-full hover:bg-hover cursor-pointer"
                                            class:hidden=move || !prev
                                        >
                                            <Icon name=String::from("angle-left")/>
                                        </div>
                                        <WindowTitle class=String::from(
                                            "w-full",
                                        )>{title}</WindowTitle>
                                    </div>
                                }
...
#[component]
pub fn NavigationSplitView(#[prop(optional)] class: String, children: Children) -> impl IntoView {
        let class = format!("NavigationSplitView flex {}", class);
...
pub fn WindowTitle(#[prop(optional)] class: String, children: Children) -> impl IntoView {
        let class = format!("{} {}", "text-md text-neutral-200 text-center py-3", class);
...
pub fn HeaderBar(
        ...
        view! {
            <div class="bg-foreground w-full rounded-t-lg flex grid gap-2 p-3" class=class>
...
pub fn Window(children: Children) -> impl IntoView {
        view! {
            <div class="relative w-full h-full bg-background shadow-xl shadow-black/50 mx-auto rounded-lg \
            border-zinc-50/10 border p-0">{children()}</div>
        }
}
...
pub fn ListBox(children: Children) -> impl IntoView {
        view! { <div class="max-h-64 overflow-auto scroll-smoth">{children()}</div> }
}
            ...
            <div class="max-w-md mx-auto">
                ...
                <div class="relative">
                    <button class="absolute inset-y-0 start-0 flex items-center ps-3 text-gray-400 hover:text-gray-200">
                        <Icon
                            class="w-4 h-4 text-neutral-400 hover:text-neutral-200".to_string()
                            name="search-loop".to_string()
                        />
                    </button>
                    <input
                        type="search"
        ...
        T: FnMut(leptos::ev::MouseEvent) + 'static,
{
        ...
        view! {
            ...
            >
                <div class="mr-3">
                    <label class="text-md text-neutral-200 block cursor-pointer">{name}</label>
                ...
                <div class="grow text-right text-sm pr-4">{address}</div>
                <button
                    type="button"
                    class="absolute inset-y-0 end-2 bg-transparent focus:outline-none p-0"
                >
        ...
        T: FnMut(leptos::ev::Event) + 'static,
{
        ...
        view! {
            <>
                ...
                <MoneyActionRow label="Price".to_string() value=price on_change=set_price/>
            </>
        }
}
        ...
        T: FnMut(Money) + 'static,
{
        ...
        let (formated_amount, set_formated_amount) =
                create_signal(number_format.format::<f64>(amount));
        ...
        view! {
            <div class="flex items-center relative bg-foreground text-neutral-200 placeholder-neutral-500 rounded-lg \
            hover:bg-hover active:bg-foreground focus:outline-none focus:ring-2 focus:ring-blue-300/50 px-3 py-0.5 w-full \
            appearance-none money-action-row">
                <div class="mr-3 py-3">
                    <label class="text-md text-neutral-200 block">{label}</label>
                </div>
                <div class="grow text-right text-sm pr-4">{formated_amount}</div>
                <button
                    type="button"
                    class="absolute inset-y-0 end-2 bg-transparent focus:outline-none p-0"
                >
        ...
        U: FnMut(leptos::ev::MouseEvent) + 'static,
{
        view! {
            <div class="flex justify-between">
                <Button label=cancel_label on_click=on_cancel/>

                <Button label=accept_label primary=true on_click=on_accept/>
            </div>
        }
}
